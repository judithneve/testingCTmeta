...
Update 29/12: done (same modification as for the previous chunk).
...
### The CTmeta.R function has been changed to:

1. Check the presence of NAs in N

2. Not have any output when the output is the same thing as the error message

3. Rephrase the error message + clean up the next line sign from it and have it actually be the next line.

### Still to do:

1. Change the documentation to specify N can be a vector of length S (see section "N as a vector").
...
Update 29/12: this was done by fixing the error message checked for in the N section.
...
1. No changes were made: all necessary changes were covered by changes made when adapting for N.

### Still to do:

1. Change the documentation to specify DeltaT can be a vector of length S (see section "DeltaT as a vector").
...
Update 29/12: the error message was modified to exclude the "Notably, currently, ..." part. This is because the error here is simply because of the length of the argument. If it is not a scalar, the error message just gets repeated pointlessly.

An error message for the case DeltaTStar is missing was also included.
...
Update 29/12: this was dealt with using the output in the previous chunk.
...
Update 29/12: this was dealt with using the output in the first chunk of this section.

## Update 29/12

### The CTmeta.R function has been changed to:

1. Return an error message when DeltaTStar is not specified

2. Have a clearer and shorter error message when DeltaTStar is not a scalar (no longer including "Notably, currently, ...")

### Still to do:

1. Change the documentation to specify DeltaTStar must be specified.
...
Update 29/12: unsure how to do this - is this necessary, i.e. an error/mistake that's likely to happen?
...
Update 29/12: same problem as above.
...
Update 29/12: made this into an error.

## Update 29/12

### The CTmeta.R function has been changed to:

1. Return an error message when Phi contains non-numerical values

2. Include a warning when Phi is an array with too many panels

3. Return an error when Phi does not have enough panels.

### Still to do:

1. Change the documentation to include an example of Phi as an array

2. Return an error when Phi is an array in which the matrices have too many columns
--> unsure how to do this; is this an error that is likely to be needed?
...
Update 03/01: changed the error message to clarify the dimensions. This will also need to be changed in Check_SigmaVAR.R
...
Update 03/01: cannot find the error message - this should be changed in Check_SigmaVAR.R
...
### The CTmeta.R file has been changed in the following ways:

1. Gives an error message when SigmaVAR has NAs

2. Gives an error message when SigmaVAR has non-numerical values

### Still to do:

1. Include an example in the documentation where SigmaVAR is an array

2. Update the error messages in Check_SigmaVAR.R (see SigmaVAR as an array with matrices an extra row)
...
### The CTmeta.R file has been modified to:

1. Include warnings and error messages

### Still to do:

1. Figure out how to navigate both SigmaVAR and Gamma being specified
...
1. Included warnings for arguments of length > 1

2. Made it into an actual error message
...
To do:
- add a warning OR have Moderators = 1 as a default if Mod is specified and Moderators isn'

Update 03/01: this will be covered by the previous warning.
...
Update 03/01: do nothing - this may already be clear enough.
...
1. Added warnings and errors

### Still to do:

1. Figure out what happens when Mod is not numerical - how does the function run? Is a warning necessary?
...
1. Made sure the error is an error

2. Included FE and RE as valid outputs

### Still to do:

1. Change documentation for FE/RE

2. What happens when RE but no BetweenLevel?
...
Update 04/01: actually, probably not needed.
...
Update 04/01: this is probably how it's meant to be. Ask more detail about what the values of BetweenLevel mean.
...
Update 04/01: this was actually already covered!
...
1. Cleaned the errors

2. Added a warning

### Still to do:

1. Figure out what the values of BetweenLevel mean - does it only matter that they are different, or does the actual number make a difference?

2. Documentation: BetweenLevel can be a factor variable
...
1. Can't print plot for moderator model with moderators
