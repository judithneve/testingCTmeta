...
summary(CTma.RE) == summary(CTma.RE.broken)
```
...
```{r}
lab.broken <- paste("test", 1:(2*2*3))
lab.broken[3] <- NA

CTma.lab.broken <- CTmeta(N,
               DeltaT,
               DeltaTStar,
               Phi,
               SigmaVAR,
...
```{r}
lab.broken <- paste("test", 1:(2*2*3))
lab.broken <- matrix(lab.broken, ncol = 1)
lab.broken
...
```{r}
lab.broken <- paste("test", 1:(2*2*3))
lab.broken <- matrix(lab.broken, ncol = 3)
lab.broken
...
funnel(CTma.lab.broken$summaryMetaAnalysis, label = 'out')
forest(CTma.lab.broken$summaryMetaAnalysis)
```
...
```{r}
lab.broken <- paste("test", 1:(2*2*3))
lab.broken <- array(lab.broken, dim = c(2, 2, 3))
lab.broken
...
```{r}
lab.broken <- paste("test", 1:(2*2*3))
lab.broken <- array(lab.broken, dim = c(1, 1, 12))
lab.broken
...
```{r}
lab.broken <- paste("test", 1:(2*2*3))
lab.broken <- array(lab.broken, dim = c(1, 4, 3))
lab.broken
...
```{r}
lab.broken <- paste("test", 1:(2*2*3))
lab.broken <- array(lab.broken, dim = c(3, 2, 2))
lab.broken
