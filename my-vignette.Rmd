...
# In this example, the same N and DeltaT(Star) values are used:
N <- matrix(c(643, 651, 473))
DeltaT <- matrix(c(2, 3, 1))
DeltaTStar <- 1
...
# Note: one can already make the time-intervals equal via the arguments DeltaTStar and DeltaT, but CTmeta can do this as well.

# In this example, the time-intervals are deliberately unequal, 
...
# Next, make Phi (the stacked lagged effects matrix)
...
# Finally, calculate the overall q-times-q (here, 2*2) lagged effects matrix overall-Phi
out_CTmeta <- CTmeta(N, DeltaT, DeltaTStar, Phi, SigmaVAR)
...
One can also have the continuous-time matrices: the lagged effects matrix Drift and the residual covariance matrix Sigma (also called diffusion matrix). Based on those matrices, Phi- and SigmaVAR-plots can be made:
